...
namespace NMLaba4
{
    class Problem2
    {
            ...
            double NormOfMatrix(List<List<double>> matrix)
            {
                ...
                double max = matrix[0].Sum(x => Math.Abs(x)); for (int i = 1; i < n; i++)
                {
            ...
            List<List<double>> MatrixSubstraction(List<List<double>> a, List<List<double>>
            b)
            {
                int n = a[0].Count;
            ...
            void Solve()
            {
                List<List<double>> matrix = new List<List<double>>(); matrix.Add(new List<double>() { 2, 1, 0 }); matrix.Add(new List<double>() { 1, 2, 1 }); matrix.Add(new List<double>() { 0, 1, 2 });
                Console.WriteLine($"First minor: {matrix[0][0]}");

                Console.WriteLine($"Second minor: {matrix[0][0] * matrix[1][1] - matrix[0][1] * matrix[1][0]}");
                var det = Det(matrix); Console.WriteLine($"Third minor: {det}");
                var norm = NormOfMatrix(matrix); Console.WriteLine($"Norm and max eigen value: {norm}");
                var B = MatrixSubstraction(new List<List<double>>()
        {
            ...
            new List<double>() { 0, 0, norm }
                ...
                Console.WriteLine(" Matrix B:"); DisplayMatrix(B);
                Console.WriteLine();
                ...
                foreach (var c in xNew) Console.Write($"{c} "); Console.WriteLine(")");
                eigenValue0 = xNew[0] / xAns[0];
                var vectorNorm = Math.Sqrt(xNew.Sum(x => Math.Pow(x, 2)));
                var normalizedVector = xNew.Select(x => x / vectorNorm).ToList(); Console.Write($"e{step} = (");
                foreach (var c in normalizedVector) Console.Write($"{c} "); Console.WriteLine(")");
                ...
                while (true)
                {
                    ...
                    Console.WriteLine($"Eigen value {step}: {eigenValue1}"); vectorNorm = Math.Sqrt(xNew.Sum(x => Math.Pow(x, 2))); normalizedVector = xNew.Select(x => x / vectorNorm).ToList(); if (Math.Abs(eigenValue1 - eigenValue0) < Eps)
                        break;
